...
- 每月 1–3 日，流水页额外展示上月月报卡片。

### 展示

- 流水：今日分析；1–3 日附加上月月报。
- 报表：今日分析（仅当前月）+ 选中月份的月报。
- 未配置 Key：一行入口「去配置」，不打断记账。
...
与现有客户端一致：Riverpod 注入、金额用整数最小单位、界面中文、测试可注入 store / Dio / chat client。密钥禁止写入日志、SnackBar 或 insight 表。
